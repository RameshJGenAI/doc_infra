...
@description('Hosting plan type for the Azure Function App. FlexConsumption provisions the FC1 serverless plan, PremiumV3 provisions a premium plan.')
@allowed([
  'FlexConsumption'
  'PremiumV3'
])
param functionAppHostingPlanType string

@description('Premium v3 SKU to use when functionAppHostingPlanType is PremiumV3.')
@allowed([
  'P0v3'
  'P1v3'
  'P2v3'
...
param premiumPlanSku string = 'P1v3'

@description('Number of pre-provisioned workers for PremiumV3 plans. Ignored when FlexConsumption is selected.')
@minValue(0)
param premiumPlanCapacity int = 1

@description('Name of the blob container that stores Flex Consumption deployment packages.')
param flexDeploymentContainerName string = 'functionappdeploy'
...
var functionHostingSku = functionAppHostingPlanType == 'FlexConsumption' ? 'FC1' : premiumPlanSku
var functionHostingKind = functionAppHostingPlanType == 'FlexConsumption' ? 'functionapp' : 'linux'
var functionHostingPlanTypeResolved = functionAppHostingPlanType == 'FlexConsumption' ? 'FlexConsumption' : 'PremiumV3'
var functionHostingCapacity = functionAppHostingPlanType == 'FlexConsumption' ? 0 : premiumPlanCapacity
var isFlexConsumptionPlan = functionHostingPlanTypeResolved == 'FlexConsumption'
...
var flexDeploymentContainers = [
  {
    name: flexDeploymentContainerName
    publicAccess: 'None'
  }
]

var storageContainers = isFlexConsumptionPlan ? concat(baseStorageContainers, flexDeploymentContainers) : baseStorageContainers
...
module procFuncStorage './modules/storage/storage-account.bicep' = {
  ...
  params: {
    ...
    publicNetworkAccess: _networkIsolation?'Disabled':'Enabled'
    allowBlobPublicAccess: false // Disable anonymous access
    containers: isFlexConsumptionPlan ? flexDeploymentContainers : []
    deleteRetentionPolicy: {
      enabled: true
      days: 7
    }
...
module hostingPlan './modules/compute/hosting-plan.bicep' = {
  ...
  params: {
    ...
    location: location
    sku: functionHostingSku
    planType: functionHostingPlanTypeResolved
    capacity: functionHostingCapacity
    kind: functionHostingKind
    tags: tags
  }
}
...
module processingFunctionApp './modules/compute/functionApp.bicep' = {
  ...
  params: {
    ...
    hostingPlanType: functionHostingPlanTypeResolved
    hostingPlanSku: functionHostingSku
    deploymentStorageAccountName: isFlexConsumptionPlan ? procFuncStorage.outputs.name : ''
    deploymentStorageContainerName: isFlexConsumptionPlan ? flexDeploymentContainerName : ''
    ...
    appSettings: [
      {
        name: 'FUNCTION_APP_PLAN_TYPE'
        value: functionHostingPlanTypeResolved
      }
    ]
  }
}

// Event Grid subscription for Flex Consumption blob triggers
// Note: This is created in postprovision.ps1 script because we need the blob extension key
// which is only available after the function app is deployed

// Assign Storage Blob Data Contributor role to system-assigned identity for deployment storage access
module deploymentStorageBlobContributor './modules/rbac/blob-contributor.bicep' = if (isFlexConsumptionPlan) {
  scope: resourceGroup
  params: {
    principalId: processingFunctionApp.outputs.systemAssignedIdentityPrincipalId
    resourceName: procFuncStorage.outputs.name
    principalType: 'ServicePrincipal'
  }
  dependsOn: [
...
// Assign Storage Queue Data Contributor role to system-assigned identity for Durable Functions queues
module funcStorageQueueContributor './modules/rbac/blob-queue-contributor.bicep' = {
  scope: resourceGroup
  params: {
    principalId: processingFunctionApp.outputs.systemAssignedIdentityPrincipalId
    resourceName: procFuncStorage.outputs.name
...
// Assign Storage Table Data Contributor role to system-assigned identity for Durable Functions tables
module funcStorageTableContributor './modules/rbac/storage-table-contributor.bicep' = {
  scope: resourceGroup
  params: {
    principalId: processingFunctionApp.outputs.systemAssignedIdentityPrincipalId
    resourceName: procFuncStorage.outputs.name
...
output FUNCTION_APP_PLAN_TYPE string = functionHostingPlanTypeResolved
